...
% f0=double(rgb2gray(imread('../../data/baboon_small.jpg')))./255;
f0=imresize(f0,0.5);
% f0 = load_image('lena');
...
%% estimation gaussian parameters on image
...
%% estimation laplace parameters on origin image
[patches_principle,coeff_proj]=princomp(resh(patch(f))');
% figure,showPatches(patches_principle);
priorModel_lap=estimateLaplace(resh(patch(f))');

%%
T=@(sigma,b)(log(2)-log(pi)+2*log(b)-2*log(sigma)+1);
T(sqrt(priorModel_gau.var),priorModel_lap.b)